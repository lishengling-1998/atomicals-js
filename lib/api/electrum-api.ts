...
export class ElectrumApi implements ElectrumApiInterface {
	...
	public async call(method, params) {
		try {
			...
			if (this.usePost) {
				response = await axios.post(`${this.baseUrl}/${method}`, {
					params,
				});
			} else {
				response = await axios.get(
					`${this.baseUrl}/${method}?params=${JSON.stringify(params)}`
				);
			}
	...
	async waitUntilUTXO(
		...
		return new Promise((resolve, reject) => {
			...
			const checkForUtxo = async () => {
				console.log('...');
				try {
					const response: any = await this.getUnspentAddress(
						address
					).catch((e) => {
						console.error(e);
						return { unconfirmed: 0, confirmed: 0, utxos: [] };
					});
					const utxos = response.utxos.sort(
						(a, b) => a.value - b.value
					);
	...
	public atomicalsGetFtInfo(
		atomicalAliasOrId: string | number
	): Promise<any> {
		return this.call('blockchain.atomicals.get_ft_info', [
			atomicalAliasOrId,
		]);
	}
	...
	public atomicalsGetLocation(
		...
		return this.call('blockchain.atomicals.get_location', [
			atomicalAliasOrId,
		]);
	}
	...
	public atomicalsGetEventHistory(
		...
		return this.call('blockchain.atomicals.get_events', [
			atomicalAliasOrId,
		]);
	}
	...
	public atomicalsList(
		...
		return this.call('blockchain.atomicals.list', [
			limit,
			offset,
			asc ? 1 : 0,
		]);
