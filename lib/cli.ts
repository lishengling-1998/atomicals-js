...
function showWalletDetails(
	...
	if (showExtra) {
		...
		for (const atomicalId in obj.atomicals_balances) {
			...
			if (
				type !== 'all' &&
				obj.atomicals_balances[atomicalId]['type'].toLowerCase() !==
					type
			) {
				continue;
			}
...
program
	...
	.action(async (address, options) => {
		try {
			...
			const result: any = await atomicals.addressInfo(
				receive.address,
				history
			);
			...
			if (alias) {
				...
				for (const walletAlias in walletInfo.imported) {
					...
					if (walletInfo.imported[walletAlias].address === identify) {
						...
						console.log(
							`Wallet Information - ${walletAlias} Address`
						);
						console.log(
							'========================================================================================================'
						);
				...
				if (!foundWallet) {
					console.log(
						`No imported wallet found with address ${identify}`
					);
				}
			} else if (show) {
				console.log(
				...
				if (all) {
					...
					if (walletInfo.imported) {
						for (const walletAlias in walletInfo.imported) {
							if (
								!walletInfo.imported.hasOwnProperty(walletAlias)
							) {
								continue;
							}
							result = await atomicals.walletInfo(
	...
	.action(async (options) => {
		try {
			...
			if (alias) {
				...
				console.log(
					`Wallet Information - Address - ${result.data?.address}`
				);
				console.log(
					'========================================================================================================'
				);
...
const resolveRealmAction = async (realm_or_subrealm, options) => {
	try {
		...
		const result: any = await atomicals.getAtomicalByRealm(
			realm_or_subrealm
		);
		console.log(JSON.stringify(result, null, 2));
	} catch (error) {
...
program
	...
	.action(async (container, options) => {
		try {
			...
			const result: any = await atomicals.getAtomicalByContainer(
				container
			);
			console.log(JSON.stringify(result, null, 2));
		} catch (error) {
...
program
	...
	.action(async (containerName, itemName, manifestFile, options) => {
		try {
			...
			const result: any =
				await atomicals.getAtomicalByContainerItemValidated(
					containerName,
					itemName,
					manifestFile
				);
		...
		try {
			...
			const modifiedStripped =
				atomicalAliasOrId.indexOf('+') === 0
					...
					: atomicalAliasOrId;
			const result = await atomicals.getRealmInfo(
				modifiedStripped,
				verbose
			);
			console.log(JSON.stringify(result, null, 2));
...
program
	...
	.option(
		'--owner <string>',
		'Show pending subrealms for an address or wallet'
	)
...
program
	...
	.option(
		'--initialowner <string>',
		'Assign claimed tokens into this address'
	)
	.option(
		'--funding <string>',
		'Use wallet alias wif key to be used for funding and change'
...
program
	...
	.option(
		'--owner <string>',
		'Use wallet alias WIF key to move the Atomicals'
	)
	.option(
		'--funding <string>',
		'Use wallet alias WIF key to be used for funding and change'
